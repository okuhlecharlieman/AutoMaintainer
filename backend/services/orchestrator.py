...
class OrchestrationEngine:
    ...
    async def approve_pipeline(self, pipeline_id: str) -> PipelineRun:
        ...
        if not pipeline.code_changes:
            logger.error("Pipeline %s: No code changes to commit", pipeline_id)
            pipeline.pr_url = "PR creation failed: Developer produced no code changes"
            pipeline.status = PipelineStatus.FAILED
            pipeline.error_message = "No code changes were generated by the Developer agent. The LLM may have returned an empty response."
        else:
